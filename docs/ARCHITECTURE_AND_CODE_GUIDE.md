...
| 앙상블 `/session/:roomId?` | [`SessionPage`](../apps/web/src/pages/SessionPage.tsx) | sidebar | 하단 기본 | 로그인·멤버십 필요. UUID `roomId` 또는 6자리 `joinCode` |
...
| 앙상블 | `SessionPage`가 workspace, 방 REST, [`RoomClient`](../apps/web/src/lib/roomClient.ts), `useMetronome.startSynchronized`를 연결한다. UUID와 6자리 `joinCode`로 REST 조회하고 WS는 canonical UUID만 쓴다. | `JOIN_ROOM` 후 PING 표본으로 offset을 추정하고 `TRANSPORT(serverStartTimeNs, revision, anchor)`를 server→performance→audio time으로 변환한다. | `POST /api/rooms`가 revision·anchor 집합과 `joinCode`를 고정한다. [`ws.py`](../apps/server/app/ws.py)는 first-frame bearer와 membership을 검증하고 [`RoomManager`](../apps/server/app/rooms.py)가 READY/START/STOP/SEEK와 roster를 관리한다. |
...
| 마디 매핑·필기 | `ScoresPage`가 권한·저장을 소유하고 [`ScoreTools`](../apps/web/src/pages/scores/ScoreTools.tsx)·[`scoreGeometry`](../apps/web/src/pages/scores/scoreGeometry.ts)·[`useScoreSurfaceGestures`](../apps/web/src/pages/scores/useScoreSurfaceGestures.ts)가 정규화 page 좌표·canonical measure를 만든다. | `scoreApi.putMeasureMap`, create/update/delete annotation. metadata+MeasureMap은 하나의 settings 요청으로 보낸다. | `MeasureMap`·`Annotation`은 expected revision을 검사한다. repertoire 전체 annotation 조회는 마디 필기를 다른 파트 map에 재투영하는 데 쓰인다. |
| 악보 재생 추적 | [`ScoreStage`](../apps/web/src/pages/scores/ScoreStage.tsx)와 [`ScoreOverlay`](../apps/web/src/pages/scores/ScoreOverlay.tsx)가 [`scoreVisibility`](../apps/web/src/pages/scores/scoreVisibility.ts)로 매핑·필기·연습 앵커를 그리고 수동 페이지 이동 후 resume CTA를 제공한다. | `useMetronome`의 audio-clock 위치를 MeasureMap page로 투영한다. | 서버는 beat를 보내지 않는다. |
