...
- S3·SMTP 준비 전의 현재 RPi는 `FMR_DEPLOYMENT_PROFILE=single_user_local`을 명시하고 UID 10001이 쓰는 전용 persistent upload volume만 사용한다. 이 프로필에서는 공개 가입·인증 재전송·비밀번호 reset·메일 기반 탈퇴 challenge를 503으로 닫고 서버 bootstrap으로 만든 active password account 하나만 허용한다. 일반 `production` 또는 암묵적 development fallback으로 이 제한을 우회하지 않는다.
...
- **운영 객체 저장소** — standard production은 `FMR_STORAGE_BACKEND=s3`, 비어 있지 않은 bucket/region, 활성화된 storage lifecycle worker 없이는 시작하지 않는다. 명시적 `single_user_local` 임시 프로필만 전용 persistent volume의 절대 경로를 허용하며 volume backup과 S3 이관 전에는 삭제하지 않는다.
...
- temporary single-user web build는 첫 진입에 운영 TODO dialog를 열고 topbar 경고 버튼으로 다시 볼 수 있게 한다. 적용 중인 로컬 volume·단일 account 제한과 남은 S3 이관·SMTP·backup·association·OMR 작업을 숨기지 않으며 standard profile 전환 때 build flag와 dialog를 함께 제거한다.
...
- `single_user_local`에서는 위 public email enrollment를 열지 않는다. `scripts/bootstrap_single_user.py`만 sole active password account를 생성·reset할 수 있고 두 번째 active identity를 거부하며, generated credential은 stdout가 아니라 mode-0600 operator file에만 기록한다.
- `bonifacio.work`의 browser production은 `FMR_SSO_ENABLED=true`와 edge `auth_request`를 함께 사용한다. `/api/auth/sso`는 proxy가 덮어쓴 non-empty `Remote-User`와 유효한 `Remote-Email`을 요구하고, 그 이메일이 이미 존재하는 active FMR 사용자와 일치할 때만 native access/refresh token을 발급하며 계정을 자동 생성하지 않는다. SSO 모드에서는 local register/login/Google/email recovery/account deletion을 모두 닫고, web logout은 앱 refresh session을 폐기한 뒤 중앙 `/sso/logout`으로 이동한다. 내부 nginx는 edge identity header를 API upstream에 명시 전달하고 origin port는 loopback에만 둔다.
...
- 임시 `single_user_local` preflight는 SMTP를 명시적으로 skipped로 기록하고 S3 대신 runtime UID가 persistent local upload root에 접근 가능한지 검사한다. 이것은 standard provider 승인이나 local file backup·S3 이관 완료를 의미하지 않는다.
