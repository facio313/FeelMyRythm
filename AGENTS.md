...
| DB | PostgreSQL (로컬 개발은 SQLite 가능) |
| 코어 | `packages/core` 순수 TS (템포맵·시계동기) |
| 오디오 | `packages/audio` Web Audio 룩어헤드 스케줄러 |
| 모바일 | Capacitor (웹 빌드 래핑) |
| 배포 | GitHub Actions → RPi `docker compose` (`DEPLOY_*` = Pilgrimage와 동일 시크릿 이름) |
...
- **`.env` 커밋 금지**. `FMR_*` / `POSTGRES_PASSWORD` / `DEPLOY_*` 하드코딩 금지.
- **배포 시크릿** — `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_KEY` (Pilgrimage와 동일 이름, 레포별로 값 등록).
